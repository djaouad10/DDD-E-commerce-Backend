...
import { EmailQueueRatingApprovedHandlerCommand } from "#/application/commands/email-queue-handlers/email-queue-rating-approved-handler.command.js";
...
describe("EmailQueueRatingApprovedHandlerService", () => {
  let container: Container;
  let service: EmailQueueRatingApprovedHandlerService;
  ...
  beforeAll(async () => {
    ...
    service = new EmailQueueRatingApprovedHandlerService(
      db,
  ...
  describe("Success Path", () => {
    test("when user, product and rating exist, it should send email and persist idempotency key", async () => {
      ...
      const command = new EmailQueueRatingApprovedHandlerCommand(
  ...
  describe("Idempotency", () => {
    test("when executed twice with same jobId, second attempt should throw", async () => {
      ...
      const rating = Rating.create(user.id, product.id, 5, null);
      await createRatingInDB(container, rating);
  ...
  describe("Transaction Safety", () => {
    test("when email gateway throws, it should rollback and NOT persist idempotency key", async () => {
      ...
      const rating = Rating.create(user.id, product.id, 2, "Bad");
      await createRatingInDB(container, rating);
